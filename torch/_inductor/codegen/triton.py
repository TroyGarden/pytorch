...
class TritonPrinter(PythonPrinter):
    ...
    def _print_OpaqueUnaryFn_cos(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.cos({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_cosh(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.cosh({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_acos(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.acos({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_sin(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.sin({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_sinh(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.sinh({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_asin(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.asin({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_tan(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.tan({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_tanh(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.tanh({self._print(expr)}{_maybe_upcast_to_fp32()})"

    def _print_OpaqueUnaryFn_atan(self, expr):
        assert len(expr.args) == 1
        return f"libdevice.atan({self._print(expr)}{_maybe_upcast_to_fp32()})"
